...
DELIMITER //
CREATE DEFINER=`kalistrat`@`localhost` FUNCTION `s_get_folder_data`(eUserLog varchar(50)) RETURNS text CHARSET utf8
begin
return(
select concat('<folder_list>'
,group_concat(concat(
'<folder_data>'
...
join users u on u.user_id=udt.user_id
where u.user_log = eUserLog
and udt.user_device_id is null
and udt.parent_leaf_id is not null
);
end//
DELIMITER ;
